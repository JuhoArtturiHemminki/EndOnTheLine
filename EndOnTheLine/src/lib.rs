...
pub fn process_dsp_stream(v_chunk: &[f32], m_chunk: &mut [u16]) {
    let simd_width = 16;
...
pub fn process_dsp_parallel(v_stream: &[f32], m_stream: &mut [u16]) {
    let simd_width = 16;
    let chunk_size = 65536;
    let mask_chunk_size = chunk_size / simd_width;

    v_stream.chunks(chunk_size)
        .zip(m_stream.chunks_mut(mask_chunk_size))
        .collect::<Vec<_>>()
        .into_par_iter()
        .for_each(|(v_chunk, m_chunk)| {
            process_dsp_stream(v_chunk, m_chunk);
        });
}
